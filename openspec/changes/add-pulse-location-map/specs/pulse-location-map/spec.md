...
When a pulse is created, the system SHALL attempt to geocode its free‑text `place` to a coordinate and persist `place_lat`, `place_lng`, and a `place_geo_status` of `resolved`, `low_confidence`, or `unresolved`. Geocoding SHALL be best‑effort and MUST NOT block or fail pulse creation: any geocoder error, timeout, empty result, or absent provider configuration resolves the pulse to `unresolved` with null coordinates. A `low_confidence` result (an ambiguous or approximate match, per the provider's confidence signal) SHALL persist its coordinates but be treated as non‑mappable downstream.

#### Scenario: A clear address resolves
- **WHEN** a pulse is created with a place the geocoder matches with high confidence
- **THEN** the pulse persists `place_lat`/`place_lng` and `place_geo_status = resolved`
...
- **THEN** the pulse is still created with `place_geo_status = unresolved` and the creator sees no failure
