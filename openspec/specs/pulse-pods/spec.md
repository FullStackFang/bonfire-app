...
The sub-group primitive on a pulse: a labelled, optionally seat-capped container (a car, a walking group, a pre-meetup) that anyone with the link can open and anyone can tap to join. Pods are optional chrome — a pulse with none renders exactly as before. Membership is one pod at a time, seat capacity is the only hard limit in the pulse system, and pods ride the existing pulse state payload and its ETag polling. Day-of ETA grouping is a read-time join over members' existing statuses, never new stored state, and podless participants are never surfaced.
