...
Capture recurrence intent after a gathering happens — the SYSTEM-THESIS §iv "again" engine. When a plan completes, attendees see a single-control afterglow screen ("do this again?"); taps accumulate on an **ember**, the per-plan recurrence object, under strict mutual-visibility rules (silence is invisible, one-sided interest is never exposed). A mutual ember can seed the next plan through the plan-coordination engine.
