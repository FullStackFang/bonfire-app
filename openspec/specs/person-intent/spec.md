...
A person intent is a single tap toward a co-attendee in the afterglow of a completed plan — "I want to see them again". Intents are pair-scoped, idempotent, and withdrawable, stored as a minimal directed row with no user-facing fields. One-sided interest is never exposed and silence is invisible: an intent reveals to no one but its author until the pair is mutual, at which point the match surfaces symmetrically to both people.
