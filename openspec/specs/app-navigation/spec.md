...
The web pulse app SHALL render the navbar once across all `/p` surfaces with three tabs — Home, Events, Groups — using flame, calendar, and people icons respectively. The active tab SHALL be determined by the current route. Home SHALL link to the existing rail at `/p`; Events and Groups SHALL link to their respective list routes.

#### Scenario: Bar persists across pulse surfaces
- **WHEN** the viewer navigates between `/p`, a pulse page, and a crew page
- **THEN** the same three-tab navbar remains fixed at the bottom of the `max-w-md` column on each

#### Scenario: Active tab tracks the route
- **WHEN** the viewer is on the Groups list route
- **THEN** the Groups tab is the ember active chip and Home and Events are inactive
