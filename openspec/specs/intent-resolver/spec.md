...
Compute draft-plan candidates for a viewer as a pure read-time resolution over their mutual embers, mutual person intents, and availability — no materialized state, cron, or timers. Compound matches rank highest, candidates speak through defaults rather than displayed facts (per `campfire-knowledge`), surfacing is pull-only, and accepting a candidate drafts a plan through the plan-coordination proposer without messaging anyone silently.
