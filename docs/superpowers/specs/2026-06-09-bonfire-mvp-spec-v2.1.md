...
- **Web push is the riskiest tech dependency and gets spiked in days 1–3, not week 3.** Expo provides no web push; it's a hand-rolled service worker + VAPID keys + a Supabase Edge Function sender. Go/no-go gate: if it's miserable on Expo web, the fallback is shipping phase 1 in `apps/web` (Next.js, where PWA push is well-trodden) reusing the shared token/type packages — the hedge this monorepo was built for. Decide by day 3, then never revisit.
